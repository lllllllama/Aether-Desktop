...
class ConfigManager:
    ...
    def validate_config(self) -> bool:
        ...
        try:
            # 验证必要的配置段是否存在
            required_sections = ['AI_CONFIG', 'DESKTOP_CONFIG', 'FILE_CONFIG', 'LOGGING_CONFIG']
            for section in required_sections:
                if not self.config.has_section(section):
                    logger.warning(f"缺少必要的配置段: {section}")
                    return False
            
            # 验证AI配置
            ai_config = self.ai_config
            if not ai_config['gemini_api_key'] or ai_config['gemini_api_key'] == 'YOUR_GEMINI_API_KEY_HERE':
                logger.warning("Gemini API密钥未配置或使用默认值")
            
            # 验证桌面路径
            desktop_config = self.desktop_config
            desktop_path = desktop_config['desktop_path']
